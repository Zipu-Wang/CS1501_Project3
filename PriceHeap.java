...
public class PriceHeap {
    ...
    // get the cheapest car in the heap
    public Car peek() {
        if (size == 0) {
            return null;
        }
    ...
    // Update Price
    public int update(String VIN, int price) {
        int index = indexMap.get(VIN);

        heap[index].setPrice(price);
    ...
    private int percolateUp(int index) {
        ...
        while (currentIndex > 0) {
            if (heap[currentIndex].getPrice() < heap[parentIndex].getPrice()) {
    ...
    private int percolateDown(int index) {
        ...
        while (currentIndex < size) {
            ...
            if (leftChildIndex < size && heap[leftChildIndex].getPrice() < heap[smallest].getPrice()) {
                smallest = leftChildIndex;
            }
    
            if (rightChildIndex < size && heap[rightChildIndex].getPrice() < heap[smallest].getPrice()) {
